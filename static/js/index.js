...
(function () {
    ...
    myechart.setOption(option);
        ...
        url : "/getsumprovince",    //请求发送到
        dataType : "json",        //返回数据形式为json

         success : function(result) {
             myechart.setOption({        //填入数据
                series: [{
                    data: (function(){
                        var rs = [];
                        for(var res in result.province){
                            rs.push({
                                name :result.province[res],
                                value:result.sum[res]
                            });
                        }
                        return rs;
                    })()
    ...
    $('.inner').on('mouseenter', '.sup li', function () {
        ...
        radomData.forEach(function (item) {
            ...
            if(i%2===0)
                ...
                html += `<li><span>${item.name}</span><span>${item.num} <s class="icon-down"></s></span></li>`;
        });
        //渲染
        $('.sub').html(html);
    });
